...
bool BinarySearchTree<Item>::find(Node<Item>* node, Item item) const {
	...
	else if(item == node->item){ return true;}
	else if(item < node->item){ return find(node->left,item);}
	else{return find(node->right,item);}
	return false;
}
...
int	BinarySearchTree<Item>::height(Node<Item>* node) const {
	...
	if( node == nullptr ){ return 0;}
		...
		Node<Item> *cur_left = node;
		while(cur_left->left != nullptr){
		cur_left = cur_left->left;
		count_left++;
		}
		Node<Item> *cur_right = node;
		while(cur_right->right != nullptr){
...
void BinarySearchTree<Item>::printTree(Node<Item>* node, int space) const {
	...
	if( node == nullptr ){return ;}
		...
		int count = 5;
		printTree(node->right,space + count);
		...
		printTree(node->left,space + count);
...
Node<Item>* BinarySearchTree<Item>::remove(Node<Item>* node, Item item) {
	...
	//********* TODO ********** 
	// std::cout <<"node->item: " << node->item << std::endl;
	...
	if(item == node->item){
       ...
       if (node->left == nullptr && node->right == nullptr) {
		...
		// case #3 our node has two children
			...
			node->right = remove(node->right,parent->item);
		}
